...
export default function Pagination({ meta }: Props) {
  ...
  const renderPages = () => {
    ...
    return visible.map((p, idx) =>
      p === 'dots' ? (
        <span key={`dots-${idx}`} className="w-9 h-9 flex items-center justify-center text-muted-foreground">
          ...
        </span>
      ) : (
        <Button
  ...
  return (
    <div className="flex justify-center items-center gap-2 mt-6">
      <Button variant="ghost" size="icon" disabled={page === 1} onClick={() => router.push(generatePageUrl(page - 1))}>
